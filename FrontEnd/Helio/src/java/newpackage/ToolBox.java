...
import parser.FileOutput;
import parser.TestSAX;
...
private static final Boolean debug = true;
    ...
    public static Stack<Stack<String>> queryWebService(Date dateStart, Date dateEnd) {
        ...
        if(debug)logger.error("entry " + dateStart + " " + dateEnd);
        ...
        //    java.lang.String dateStartString = "2004.01.01 00:00:00";
          //  java.lang.String dateEndString = "2004.01.02 00:00:00";
            ...
            if(debug)logger.error("return result number =" + stack.size());
            return stack;
            
        } catch (Exception ex) {
            if(debug)logger.error("catch " + ex);
            if(debug)logger.error("return " + null);
            return null;
        }
    ...
    public static String getFortmattedDate(Date date) {
        ...
        Logger logger = Logger.getLogger(CLASS + " " + methodName);
        if(debug)logger.error("entry " + date);

        String DATE_FORMAT = "yyyy.MM.dd hh:mm:ss";
        SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
        if(debug)logger.error("return " + date);
