...
import ch.i4ds.helio.ServiceLocator;
...
public class Workflows
{
  ...
  public void setServiceLocator(ServiceLocator _serviceLocator)
  {
  ...
  public String runInitialWorkflow(
      ...
      @WebParam(name="service_set") final String _service_set) throws java.lang.Exception
  {
    ...
    //query the HEC
    String sql_output=new HECService(
        new URL(serviceLocator.getServiceLocation(_service_set,"hec")),
        new QName("http://dpas.helio.i4ds.ch/", "HECService")
      ).getHECPort().sql(sql_string);
     ...
     * can get the XML-serialized result of a web service invocation, because the workflow
    ...
    for(int i=0;i<startDates.size();i++)
    {
      ...
      ecs_hessi_ec.submit(new Callable<String>(){
        ...
        {
          FrontendFacade port=new FrontendFacadeService(
              new URL(serviceLocator.getServiceLocation(_service_set,"frontend")),
              new QName("http://core.helio.i4ds.ch/", "FrontendFacadeService")
          ).getFrontendFacadePort();
