...
$sql_string="select * from hec_catalogue WHERE status NOT LIKE 'deleted' order by sort;";
  ...
  for ($mo=1;$mo<=12;$mo++) {
    ...
    foreach ($cats as $cat) {
      if (DEBUG==0) {
        ...
        if (strpos($cat,"ars_earth_icme")>0) $sql_string2="select count(*) from $cat where time_start_earth>='$y-$mo-1' AND time_start_earth<='$y-$mo-$dd[$mo]';";
