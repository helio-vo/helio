<?php
	...
	function parse_date($year,$datetime)
	{
		...
		$z = preg_split('/\/|( +)/',$datetime);
                echo "KMB_YEAR_DT:$year,$datetime\n";

                echo "KMB_Z:$z[1],$z[1],$z[2]\n";

//		$t = sprintf("%02d/%02d/%04d %02d:%02d:00",$z[1],$months[$z[0]],$year,substr($z[2],0,2),substr($z[2],3,2));

//KMB: Added some logic on the parsing.  The 3,2 was wrong in almost all cases.   But there does seem to be a special case where an odd character is
//in the '0' location shifting everything. i.e. ~ Now it checks the first two digits of time is numeric or shift it.
if (is_numeric(substr($z[2],0,2))) {
		$t = sprintf("%04d/%02d/%02d %02d:%02d:00",$year,$months[$z[0]],$z[1],substr($z[2],0,2),substr($z[2],2,2));
}else {
		$t = sprintf("%04d/%02d/%02d %02d:%02d:00",$year,$months[$z[0]],$z[1],substr($z[2],1,2),substr($z[2],3,2));
}
...
//	$tempdir = "/var/www/html/sec/temp";
	...
	// get files from HTTP
	exec ("wget http://www.mssl.ucl.ac.uk/~rdb/helio-hec/gsfc-seps.html");
	copy ("gsfc-seps.html",$tempdir."/seps.html");
	unlink ("gsfc-seps.html");
