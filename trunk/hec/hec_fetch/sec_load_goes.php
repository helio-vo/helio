<?php
	...
	# sec_load_goes.php
	# 06-jul-2004, last 12-oct-2004
	# ---------------------------------------------
	# read the GOES xray flare data
	# ftp://anonymous@ftp.ngdc.noaa.gov/STP/SOLAR_DATA/SOLAR_FLARES/FLARES_XRAY
	# =============================================
	define("DEBUG","0");
...
//	$tempdir = "/var/www/html/sec/temp";
	...
	if (DEBUG==0) {
		...
		exec ("/bin/rm temp/goes/XRAY*");
		//exec ("wget -r -nd -P temp/goes -N -A 'XRAY[1-9]*','xray*' ftp://anonymous@ftp.ngdc.noaa.gov/STP/space-weather/solar-data/solar-features/solar-flares/x-rays/goes/2013\*");
		//exec ("wget -r -nd -P temp/goes -N -A 'XRAY[1-9]*','goes-xray_20*' ftp://anonymous@ftp.ngdc.noaa.gov/STP/space-weather/solar-data/solar-features/solar-flares/x-rays/goes/2013\*");
		exec ("wget -r -nd -P temp/goes -N -A 'XRAY[1-9]*','goes-xray_20*' ftp://anonymous@ftp.ngdc.noaa.gov/STP/space-weather/solar-data/solar-features/solar-flares/x-rays/goes/2012\*");
		exec ("cat temp/goes/goes* temp/goes/XRAY* > all_xray.txt");
...
//        $buf[$i++] = trim(substr($buffer,59,4));
        ...
        for ($k=2;$k<=11;$k++) {
            if (($buf[$k]=="") or (strpos($buf[$k],"//://:")>0)) $buf[$k]="\N";
            $out.="\t".$buf[$k];
        }
        $out.="\n";
