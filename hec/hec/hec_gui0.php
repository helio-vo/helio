...
//    $groupscolor = array('ccdddd','ccbbbb','ccaaaa','cccc99','ivory');
    ...
    while ($r = pg_fetch_array($result)) {
        ...
        echo('<td><input id="chkcat'.$r['cat_id'].'" name="'.$r['name'].'" value="istable" type="checkbox"></td>');
        echo('<td>'.$r['description'].'</td>');
        echo('<td>'.$r['type'].'</td>');
        echo('<td>'.$r['status'].'</td>');
        echo('<td><a href="'.$r['url'].'" target="_blank">URL</a></td>');
        echo('<td>'.$r['timefrom'].'</td>');
        $timeto = $r['timeto'];
...
*/
    echo '<INPUT id="idmetadatasearch" TYPE="hidded" NAME="metadatasearch2">';
