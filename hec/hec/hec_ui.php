<?php	
		...
		if ($result) {
			...
			while ($r = pg_fetch_row($result)) {
				...
				$catlist = array_merge($catlist,$temparr);
                                $temparr2 = array($r[1] => $r[6]);
                                $groupslist = array_merge($groupslist,$temparr2);
			}
		}
	}
	...
	foreach ($catlist as $k=>$v) {
          if ($groupslist[$k]!=$lastgroupslist) echo "<option disabled='disabled'>---------</option>";
          $lastgroupslist = $groupslist[$k];
...
echo '<a href="http://140.105.77.30/hec/stfc/HEC_Lists.html">.</a>';
	echo (sec_footer());
?>
