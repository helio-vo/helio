<?php
  ...
  echo '<tr valign="middle"><td colspan=3 align="center" cellpadding="0"><b>Recent Changes</b>&nbsp;<a href="http://helio-cdaws.blogspot.com/search/label/HEC" target="hio_blog"><img src="blogspot-favicon_sm.jpg" title="CDAW Blog topic on the HEC" border=0></a>
</td></tr>';
...
//  echo '<P>';
  ...
  echo '</td>';
    ...
    echo '<a href="hec-crosstable.html" target="_blank"><img src="table.jpg" heigth="%5" width="%5"></a>';
    echo '</td></tr>';
...
*/
    ...
    $sql_string="SELECT cat_id,name,description,type,status,url,hec_groups_id,bg_color,longdescription,
     ...
     FROM hec_catalogue ORDER BY sort;";// WHERE hec_groups_id = $g ORDER BY cat_id;";
    $result = pg_exec($dbconn,$sql_string);
    if (!$result) {
      echo "error\n";
    }
...
//    echo '&nbsp;<img src="info_icon.gif" title="Select catalogues to get events from and fetch data" onmouseover="return mouse();" " >';
    echo '</td>';
    ...
    echo ' <table bgcolor="#F8FDBA" width=450 cellpadding=6 cellspacing=0 border=1 >';
    echo ' <tr> <td>Closed</td><td>	The catalogue updating has definitively stopped and will not be resumed. This category includes lists derived from Journal articles. </td> </tr>';
    echo ' <tr> <td>Inactive</td><td>	The catalogue could be updated but it is not clear the provider intends to do this.</td> </tr>';
    echo ' <tr> <td>Static</td><td>	Updating of the catalogue is sporadic because of issues either on the provider side or on the HEC service side.</td> </tr>';
    echo ' <tr> <td>Active</td><td>	The catalogue is being updated (automatically) either on a routine or on an irregular basis.</td> </tr>';
    echo ' </table> </div>';
    ...
    echo ('<tr><th class="Select">Select</th><th class="Catalogue">Catalogue Description</th><th class="Type">Type</th><th class="Status">Status<img src="info_icon.gif" onmouseover="hoverover(event,\'status\');" onmouseout="hoverout(event,\'status\');"></th><th class="Source">Source</th><th class="From">From</th><th class="To">To</th><th class="Info">Info</th></tr>'."\n");
    echo '</thead>';
...
//        echo('<td bgcolor="white"><a href="http://140.105.77.30/hec/stfc/HEC_ListsAll.html#'.$r['name'].'" target="_blank" >Info</a></td>');
        echo("<td bgcolor=\"white\">
          <INPUT type=\"button\" value=\"Info\" onClick=\"{mywindow = window.open('stfc/HEC_ListsAll.html#".$r['name']."','mywindow','width=800,height=400,scrollbars=1');mywindow.moveTo(100, 100);mywindow.focus();return false;}\"></td>");
...
*/
    ...
    echo('<hr>'."\n");
    echo('<a href="hec_gui_free.php" target="_blank">Free SQL search</a>'."\n");
    echo('&nbsp;&nbsp;&nbsp;<a href="Browser_Compatibility.php" target="_blank">Browser Compatibility</a>'."\n");
/*
