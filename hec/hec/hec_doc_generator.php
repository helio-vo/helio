<?php
  ...
  for ($g=1; $g<=5; $g++) {
    ...
    $sql_string="SELECT name,description,url,bg_color,longdescription FROM hec_catalogue WHERE hec_groups_id=$g;";
    $result = pg_exec($dbconn,$sql_string);
    if ($result) {
      while ($r = pg_fetch_array($result)) {
...
/*
	$sql_string="select * from hec_catalogue;";
	$result = pg_exec($dbconn,$sql_string);
	if ($result) {
...
*/
  ...
  for ($j=1; $j<=5; $j++) { 
    $s .= "<table border=1>\n";
    // read database for table list
    $sql_string="select * from hec_catalogue WHERE hec_groups_id=$j;";
    $result = pg_exec($dbconn,$sql_string);
      //if ($j==1)
      //  for ($ii=$i; $ii>0; $ii--)
