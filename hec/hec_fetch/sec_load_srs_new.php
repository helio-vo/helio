...
//.ftp_pasv($conn,true);
	...
	//}
	//ftp_close($conn);
$cmd = 'wget -N -P srs_data -nd -np -r "ftp://ftp.swpc.noaa.gov/pub/warehouse/' . $curyear . '/SRS"'; 
echo $cmd;
shell_exec($cmd);
$list = scandir("srs_data");
} #debug==0
	...
	foreach ($list as $file) {
	 if (strlen($file)>4) {
		echo "$tempdirsrs/$file\n";
		$f2 = fopen("$tempdirsrs/$file",'r');
		$i++;#files counter
                if($f2) {
		do {
			$buffer = fgets($f2);
...
//			$out.="\t\N";#end_time null
  ...
  echo "Could not process: $tempdirsrs/$file\n";                  
                }
	 }#if strlen($file)
	}#foreach
